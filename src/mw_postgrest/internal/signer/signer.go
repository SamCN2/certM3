...
import (
	...
	"github.com/ogt11/certm3/mw_postgrest/internal/pkg/metrics"
)
...
// getUserGroups retrieves the user's groups from the PostgREST backend API
func (s *Signer) getUserGroups(username string) ([]string, error) {
	// Call the RPC endpoint for the get_user_groups function
	endpoint := fmt.Sprintf("%s/rpc/get_user_groups", s.config.AppServer.BackendAPIURL)
	...
	// The signer does not have access to the SPA's JWT, so it can't generate a backend JWT.
	// This implies that the get_user_groups function must be callable by the 'anonymous' role
	// or another role that the signer can use. For now, we assume it's public.
	// A more secure implementation might involve the middleware passing a temporary, single-purpose token
	// to the signer, which the signer could then forward to the backend.
	...
	if resp.StatusCode != http.StatusOK {
		return nil, fmt.Errorf("backend API returned status %d for groups lookup", resp.StatusCode)
	}
